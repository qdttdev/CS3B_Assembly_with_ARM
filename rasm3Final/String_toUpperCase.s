...
@ Subroutine String_toUpperCase accepts the address of a string and then
@ converts all of its chars to Upper case.
@
...
@
@    R0: Address of a string with Upper case chars.
...
loop:
		...
		cmp		r5, #0x61		@ check to see if r5 is greater then 0x61
...
replace:
		...
		cmp		r5, #0x7A		@ check to see if r5 is greater then 0x7A
		bgt		dontDoAnything	@ branch to dontDoAnything if r5 is greater then 0x5A

		sub r7, r5, #0x20		@ sub 0x20 to get the upper case of the letter
